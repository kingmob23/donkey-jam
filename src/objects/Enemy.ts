...
export class Enemy extends Phaser.GameObjects.Image {
    ...
    constructor(scene: Phaser.Scene, x: number, y: number, player: Phaser.GameObjects.Image) {
        super(scene, x, y, 'raider'); // Используем новую текстуру 'raider'
        this.player = player;
        ...
        this.bang = scene.add.image(this.x, this.y, 'raider_attack');
    ...
    public hit() {
        ...
        if (this.health <= 0) {
            ...
            return true; // Возвращаем true, чтобы показать сообщение
        }
        return false;
    }
